-- =============================================================================
-- Ofertas: 3 minutos visibles → 1 minuto de pausa → reintento
-- Solo si el pedido sigue en estado NUEVO (pendiente).
-- Si caja/admin pasa a aceptado / confirmado / cocina / delivery / entregado /
-- cancelado: se cancelan ofertas abiertas y no se reintenta.
-- =============================================================================

ALTER TABLE public.ep_dispatch_settings
  ALTER COLUMN offer_ttl_seconds SET DEFAULT 180;

ALTER TABLE public.ep_dispatch_settings
  ALTER COLUMN retry_after_seconds SET DEFAULT 240;
...
COMMENT ON COLUMN public.ep_dispatch_settings.offer_ttl_seconds IS
  'Segundos que la oferta permanece en el celular del repartidor (3 min).';
COMMENT ON COLUMN public.ep_dispatch_settings.retry_after_seconds IS
  'Segundos desde offered_at para re-ofertar: 180 TTL + 60 pausa = 240.';

-- Reusar búsqueda con TTL 180 (diagnóstico incluido)
...
BEGIN
  ...
  ttl := COALESCE(settings.offer_ttl_seconds, 180);
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE
      WHEN offered > 0 THEN format('Oferta enviada a %s repartidor(es) · %s s para aceptar', offered, ttl)
      WHEN online_n = 0 THEN 'No hay repartidores en Disponible. Pídele que pulse Conectarme en la app.'
      WHEN skipped_gps > 0 THEN 'El repartidor está en línea pero su GPS aún no llega al servidor.'
...
BEGIN
  ...
  -- Si caja/admin ya movió el pedido, cerrar ofertas
  FOR job IN
  ...
  LOOP
    ...
    ttl := COALESCE(settings.offer_ttl_seconds, 180);
    retry_after := COALESCE(p_retry_after_seconds, settings.retry_after_seconds, 240);

    -- Esperar TTL (3 min visible) + pausa (1 min) desde offered_at
    ...
    FOR drv IN
      ...
      WHERE d.admin_status = 'approved'
        ...
        AND public.ep_driver_location_is_fresh(d.id, 180)
    LOOP
